...
if (Object.keys(s.byHost).length > 1) console.log(`  hosts: ${dist(s.byHost)}`)
console.log(`  estimated savings vs all-heavy: ${pct(s.savingsRate)} (price table ${config.priceTable ? 'config override' : `v${PRICE_TABLE.version}, ${PRICE_TABLE.asOf}`})`)
console.log(`  Hard-tier tasks downgraded (budget mode): ${pct(s.hardDowngraded)}`)
console.log(`  self-healed retries (escalated back to heavy): ${s.escalations}`)
console.log(`\nLog: ${logPath}`)
