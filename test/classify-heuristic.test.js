...
test('explicit model is respected as an override (never routed)', () => {
  const r = classifyHeuristic({ subagent_type: 'Explore', prompt: 'find foo', model: 'heavy' })
...
test('non-Explore agents outside general-purpose are never downgraded', () => {
  for (const t of ['Plan', 'code-reviewer', 'custom', 'Explore-writer']) {
    ...
    assert.equal(r.skip || r.model, r.skip || 'heavy')
    if (!r.skip) assert.equal(r.model, 'heavy', `${t} should stay on heavy`)
  }
})
...
test('general-purpose: short read-only search → light', () => {
...
test('general-purpose: mechanical verb → mid', () => {
...
test('general-purpose ADVERSARIAL: search + hard verb → stays on heavy', () => {
  const traps = [
  ...
  for (const prompt of traps) {
    ...
    assert.equal(r.model, 'heavy', `trap must stay on heavy: "${prompt.slice(0, 40)}…"`)
  }
})

test('general-purpose ADVERSARIAL: long or vague prompts → heavy', () => {
  const vague = [
...
test('invariant: the hook never escalates or touches a non-downgrade case', () => {
  // Anything that doesn't confidently downgrade must yield heavy, not mid/light.
  const cases = [
