// Curated, versioned model registry — the tier→slot resolution layer shared
// by every host adapter (spec #10), plus the slot VOCABULARY itself.
//
// Hand-curated on purpose: a drifted table routes wrong, so the registry is
// the truth source, versioned and dated like the price table, and runtime
// provider auto-discovery may only supplement it — never override curated
// anchors. Each provider gets one ORDERED candidate pool per tier; pool
// order IS the quality order, leader first.
//
// ── Slot vocabulary (v2) ────────────────────────────────────────────────────
// The pipeline's model names are HOST-NEUTRAL SLOTS — light / mid / heavy —
// not any vendor's model ids. Each host adapter resolves a slot to something
// concrete at its own boundary:
//   Claude Code:  wireName() → 'haiku' / 'sonnet' / 'opus' in updatedInput
//   Codex:        the user's codex.models config (or a pinned tier agent)
//   Pi:           the provider's own model registry (via its wire alias)
// The pre-v0.5.0 names (haiku/sonnet/opus) remain legal synonyms in user
// config and in old log entries — canonicalSlot() folds them in so a config
// or a shared log written before the rename keeps working unchanged.
//
// `prefer` implements the tier-internal specialty tiebreaker under the
// formal no-quality-loss rule: a candidate may replace the pool leader only
// when it scores at least as high as the leader on the preferred dimension.
// Among qualifying candidates the higher score wins; ties go to the cheaper
// model, then to curated order.

export const MODEL_REGISTRY = Object.freeze({
  version: 2,
  asOf: '2026-09-11',
  // Strength scores are relative within the curated lineup (1–5), not
  // absolute benchmarks. Context lengths in thousands of tokens.
  models: Object.freeze({
    light: Object.freeze({
      relativeCost: 0.1,
      contextK: 200,
      strengths: Object.freeze({ reasoning: 3, code: 3, longContext: 3, speed: 5 }),
    }),
    mid: Object.freeze({
      relativeCost: 0.3,
      contextK: 200,
      strengths: Object.freeze({ reasoning: 4, code: 4, longContext: 4, speed: 4 }),
    }),
    heavy: Object.freeze({
      relativeCost: 1.0,
      contextK: 200,
      strengths: Object.freeze({ reasoning: 5, code: 5, longContext: 5, speed: 2 }),
    }),
  }),
  providers: Object.freeze({
    anthropic: Object.freeze({
      pools: Object.freeze({
        Trivial: Object.freeze(['light']),
        Routine: Object.freeze(['mid']),
        Hard: Object.freeze(['heavy']),
        Unknown: Object.freeze(['heavy']),
      }),
    }),
  }),
})

// Pre-v0.5.0 slot names → canonical slots. Legacy forever: user configs and
// shared logs written before the rename must survive an upgrade untouched.
const LEGACY_SLOT_ALIASES = Object.freeze({ haiku: 'light', sonnet: 'mid', opus: 'heavy' })

/**
 * Fold a legacy alias into its canonical slot. Everything else — canonical
 * slots, "never", concrete model ids, non-strings — passes through verbatim.
 * @param {string} name
 * @returns {string} the canonical name (possibly unchanged)
 */
export function canonicalSlot(name) {
  return typeof name === 'string' ? (LEGACY_SLOT_ALIASES[name] ?? name) : name
}

// Provider-native wire names for each slot — what the host actually accepts.
// Non-slot strings (user-pinned concrete ids) have no wire name; adapters
// pass those through verbatim.
const WIRE_NAMES = Object.freeze({
  anthropic: Object.freeze({ light: 'haiku', mid: 'sonnet', heavy: 'opus' }),
})

/**
 * The provider-native model id a slot resolves to on the wire (Claude Code's
 * updatedInput value, Pi's registry-lookup alias). undefined for non-slot
 * strings or providers without curated wire names — callers then pass the
 * original string through verbatim.
 * @param {string} slot
 * @param {{provider?: string}} [options]
 * @returns {string|undefined}
 */
export function wireName(slot, { provider = 'anthropic' } = {}) {
  return WIRE_NAMES[provider]?.[slot]
}

/**
 * Resolve a tier to a concrete slot from the registry.
 *
 * @param {'Trivial'|'Routine'|'Hard'|'Unknown'} [tier]
 * @param {{provider?: string, prefer?: string|null, registry?: object}} [options]
 *   `prefer` names a strengths dimension (e.g. 'longContext', 'speed') for
 *   the tier-internal tiebreaker.
 * @returns {string|null} slot name, or null when the provider (or tier pool)
 *   isn't curated — callers fall back to the policy's own tier map.
 */
export function resolveModel(tier, { provider = 'anthropic', prefer = null, registry = MODEL_REGISTRY } = {}) {
  const pools = registry.providers?.[provider]?.pools
  // Unknown or missing tier → the Unknown pool (same normalization as the
  // policy's safe default).
  const pool = Array.isArray(pools?.[tier]) ? pools[tier] : pools?.Unknown
  if (!Array.isArray(pool) || pool.length === 0) return null
  const leader = pool[0]
  if (!prefer) return leader

  const score = (m) => registry.models?.[m]?.strengths?.[prefer]
  const cost = (m) => registry.models?.[m]?.relativeCost ?? Number.POSITIVE_INFINITY
  const leaderScore = score(leader)
  if (typeof leaderScore !== 'number') return leader

  let best = leader
  let bestScore = leaderScore
  for (const candidate of pool.slice(1)) {
    const s = score(candidate)
    if (typeof s !== 'number' || s < leaderScore) continue // no-quality-loss rule
    if (s > bestScore || (s === bestScore && cost(candidate) < cost(best))) {
      best = candidate
      bestScore = s
    }
  }
  return best
}
